...
1. **Файловый режим** - логи пишутся только в файл:
   - Задача: `run-journal-file`
   - Параметры: `log.txt MEDIUM`

2. **Сетевой режим** - логи пишутся и в файл, и отправляются по сети:
   - Сначала запустите `run-stats-collector` (сервер статистики)
   - Затем `run-journal-socket` (клиент с сокетами)
...
3. Сетевой режим (в разных терминалах):
```bash
